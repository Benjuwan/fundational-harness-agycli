...
2. 承認後、検証専用サブエージェント（`qa-auditor`）を直接起動（標準のサブエージェント起動ツールを利用）して、視覚的突合監査を行わせる。
   - **ミッション・ブリーフ（指示内容）**:
...
2. 承認後、検証専用サブエージェント（`qa-auditor`）を直接起動（標準のサブエージェント起動ツールを利用）して、構造化Markdownの品質を最終監査する。
   - **ミッション・ブリーフ**:
