...
- 参照リポジトリ: https://github.com/keitakn/engineering-skills/tree/main/.claude/skills/explain-visually
...
# Chrome の実行パス。環境変数 CHROME_PATH があればそれを優先し、無ければ macOS の既定パスを使う
CHROME = (
...
def main() -> int:
    ...
    parser.add_argument(
        "--width", type=int, default=1250, help="ビューポート幅（既定: 1250）"
    )
    parser.add_argument(
        "--wait",
        type=int,
        default=25000,
        help="描画を待つ仮想時間のミリ秒（既定: 25000）",
    )
    parser.add_argument(
        "--timeout",
        type=int,
        default=40,
        help="Chrome1回あたりの打ち切り秒数（既定: 40）",
    )
    ...
    if not Path(CHROME).exists():
        print(
            json.dumps(
                {"ok": False, "error": f"Google Chrome が見つかりません: {CHROME}"},
                ensure_ascii=False,
            )
        )
        return 1
    ...
    with tempfile.TemporaryDirectory(prefix="explain-visually-") as tmp:
        ...
        if not shot.exists():
            print(
                json.dumps(
                    {"ok": False, "error": "スクリーンショットを生成できなかった"},
                    ensure_ascii=False,
                )
            )
            return 1
